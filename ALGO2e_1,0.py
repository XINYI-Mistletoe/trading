...
while True:
    ...
    while True:
        ...
        if (buy_orders == []) and (sell_orders == []) and (net_position(i) == 0):
            ...
            if (abs(new_ask_price - sell_limit_p) <= 0.02) and (t <= 7):
                pass
            else:
            ...
            if (abs(new_bid_price - buy_limit_p) <= 0.02) and (t <= 7):
                pass
            else:
                while (get_open_orders(ticker_list[i]) != ([],[])):
                    resp = s.post('http://localhost:9999/v1/commands/cancel', params = {'ticker': ticker_list[i]})
                ...
                while abs(x) > 5000:
                    new_bid_price, new_ask_price = get_bid_ask(ticker_list[i])
                    resp = s.post('http://localhost:9999/v1/orders', params = {'ticker': ticker_list[i], 'type': 'LIMIT', 'quantity': 5000, 'price': new_ask_price+spread(i,t), 'action': 'BUY'})
                    x = abs(x)-5000
            ...
            if t <= 14:
                ...
                if x < 0:
                    while abs(x) >= 5000:
                        resp = s.post('http://localhost:9999/v1/orders', params = {'ticker': ticker_list[i], 'type': 'MARKET', 'quantity': 5000, 'action': 'BUY'})
                        print(resp.ok)
                        x = abs(x)-5000
                    ...
                    while abs(x) >= 5000:
                        resp = s.post('http://localhost:9999/v1/orders', params = {'ticker': ticker_list[i], 'type': 'MARKET', 'quantity': 5000, 'action': 'SELL'})
                        x = abs(x)-5000
